...
class ZapadOST extends React.Component {
  constructor(props) {
    super(props)
  }
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    const chart = this.refs['chart_ost_zapad_prehlad'].getChart()
    ...
    this.props.zapad.pocasie_skutocnost.map( row => { teplota.push([ row['datum'], row['hodnota'] ]) })
    this.props.zapad.vlastne_vykon.map( row => { vlastne_vykon.push([ row['datum'], parseFloat((row['hodnota'] / 1000).toFixed(2)) ]) })
    this.props.zapad.cudzie_vykon.map( row => { cudzie_vykon.push([ row['datum'], parseFloat((row['hodnota'] / 1000).toFixed(2)) ]) })
    this.props.zapad.vlastne_prenos.map( row => { vlastne_prenos.push([ row['datum'], row['hodnota'] ]) })
    this.props.zapad.cudzie_prenos.map( row => { cudzie_prenos.push([ row['datum'], row['hodnota'] ]) })
  ...
  render() {
    ...
    return (
      <div>
        <ReactHighcharts config={chart} ref={'chart_ost_zapad_prehlad'} isPureConfig />
      </div>
    )
  }
}
